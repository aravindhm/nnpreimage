function exp = exp_all(pretend, subset)
...
for i = 1:numel(models)
  ...
  mega = strcmp(models{i}, 'imagenet-caffe-alex');
...
%  exp = horzcat(exp, setup_category_variants(modelPath, statsPath)) ;
  ...
  if ~mega, continue; end
  exp = horzcat(exp, setup_inversion_multival(modelPath, statsPath));
end
...
% --------------------------------------------------------------------
function exp = setup_inversion_multival(modelPath, statsPath)
% --------------------------------------------------------------------
...
prefix = 'inversion-megamulti-val160';
images = dir('data/pics/val/*.JPEG') ;
images = fullfile('val', {images.name}) ;
images = images(1:24);
exp0.numRepeats = 4 ;
...
for i = 1:numel(images)
  [~,base,ext] = fileparts(images{i}) ;
  for l = [18, 16, 20, 17, 19] %1:numel(net.layers)-1
    for s = [1]% 100 300 20]
      exp0.objectiveWeight = s ;
      exp0.layer = l ;
      for repeat_no = 1:6
        exp0.resultPath = sprintf('data/%s/%s/%s/%s', ...
          prefix, name,  base, ...
          sprintf('layer%02d-repeat%d-str%04.1f', ...
          exp0.layer, repeat_no, exp0.objectiveWeight)) ;
        exp0.initialImage = sprintf('data/pics/%s', images{i}) ;
        exp{end+1} = exp0 ;
      end
    end
